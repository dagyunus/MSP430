...
static i2c_status_t i2c_start_job(const I2C_Bus *bus, uint8_t dev_addr,
                                  ...
                                  uint8_t rx_len)
{
    ...
    if ((ctx == 0) || (tx == 0) || (tx_len == 0U) || (tx_len > I2C_MAX_TX_BYTES) ||
        (rx_len > I2C_MAX_RX_BYTES))
    {
...
i2c_status_t i2c_async_write_reg_multi(const I2C_Bus *bus, uint8_t dev_addr,
...
i2c_status_t i2c_async_read_bytes(const I2C_Bus *bus, uint8_t dev_addr,
...
static __inline void i2c_isr(i2c_context_t *ctx, uint16_t iv)
{
