...
static void vout_ctrl_high(void)
{
    P4OUT |= VOUT_PWR_CTRL_PIN;
}

static void vout_ctrl_low(void)
{
    P4OUT &= ~VOUT_PWR_CTRL_PIN;
}

static void fault_flag_clear(void)
{
    P4OUT |= EXT_FAULT_FLAG_PIN;
}

static void fault_flag_active(void)
{
    P4OUT &= ~EXT_FAULT_FLAG_PIN;
}
...
void Protection_Task(void)
{
    ...
    switch (g_sys_state)
    {
        ...
        case STATE_STANDBY:
            vout_ctrl_high();
            /* Dışarıdan ON/OFF kontrolü: EXT_ON_OFF_CTRL_PIN LOW ise aç */
        ...
        case STATE_STARTING:
            ...
            if (v_in > LIMIT_UVLO_REC_V && v_in < LIMIT_OVP_REC_V &&
        ...
        case STATE_NORMAL:
            ...
            if (v_in < LIMIT_UVLO_V || v_in > LIMIT_OVP_V ||
                v_protected < LIMIT_UVLO_V || v_protected > LIMIT_OVP_V ||
                temp > LIMIT_TEMP_OTP) {
                g_fault_strikes++;
                g_sys_state = (g_fault_strikes >= 3) ? STATE_LOCKED : STATE_FAULT;
            }
        ...
        case STATE_FAULT:
            ...
            if ((P4IN & EXT_ON_OFF_CTRL_PIN) == 0) {
                if (v_in > LIMIT_UVLO_REC_V && v_in < LIMIT_OVP_REC_V &&
                    ...
                    if ((P8IN & LTC4364_FLT_PIN) &&
                        (P8IN & TMP101_ALERT_PIN) &&
                        (P4IN & TMP101_ALERT_ISO_PIN)) {
                        g_sys_state = STATE_STARTING;
                    }
                }
            } else {
