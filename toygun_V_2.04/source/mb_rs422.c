...
#define VOUT_270_IMON_ZERO_V    0.32f
#define VOUT_270_IMON_SENS_V_A  0.264f
...
void MB_RS422_UpdateTable(void)
{
    ...
    if ((P4OUT & EXT_FAULT_FLAG_PIN) == 0) MB_RS422_Table.Data.Status.Bits.Ext_Fault = 1;
    ...
    switch(g_sns_state) {
        ...
        case S_TMP_IN_REQ:
            ...
            else if (st != I2C_ERR_BUSY) { MB_RS422_Table.Data.Diag.Bits.TMP_In_Fail = 1; g_sns_state = S_TMP_ISO_REQ; }
            break;
        ...
        case S_TMP_ISO_REQ:
            ...
            else if (st != I2C_ERR_BUSY) { MB_RS422_Table.Data.Diag.Bits.TMP_Iso_Fail = 1; g_sns_state = S_ADS1_CFG_REQ; }
            break;
        ...
        case S_ADS1_CFG_WAIT:
            ...
            if (st == I2C_OK) { g_sns_timer = 120; g_sns_state = S_ADS1_RD_REQ; } // Dönüşüm bekleme
            else if (st != I2C_ERR_BUSY) { MB_RS422_Table.Data.Diag.Bits.ADS_Fail = 1; g_sns_state = S_ADS2_CFG_REQ; }
            break;
        ...
        case S_ADS2_CFG_WAIT:
            st = i2c_async_poll(&g_i2c_ucb0, 0, &rx_len);
            if (st == I2C_OK) { g_sns_timer = 120; g_sns_state = S_ADS2_RD_REQ; }
            else if (st != I2C_ERR_BUSY) { MB_RS422_Table.Data.Diag.Bits.ADS_Fail = 1; g_sns_state = S_DS1682_REQ; }
            break;
        ...
        case S_DS1682_REQ:
            st = i2c_async_read_bytes(&g_i2c_ucb2, DS1682_ADDR, 0x01, 4); // 0x01 = DS1682_REG_ETC
            if (st == I2C_OK)
                g_sns_state = S_DS1682_WAIT;
            else if (st != I2C_ERR_BUSY) { MB_RS422_Table.Data.Diag.Bits.DS1682_Fail = 1; g_sns_state = S_ADC_READ; }
            break;
